...
export async function generateMetadata({
  ...
  return generatePositionDigitMetadata("birthday", digit);
}

export default async function BirthdayDigitPage({
  params,
}: {
  params: Promise<{ digit: string }>;
}) {
  const { digit } = await params;
  return <PositionDigitPage positionSlug="birthday" digitParam={digit} />;
}
